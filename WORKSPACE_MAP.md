...
| 指针目标 | 内容 | 与本工作区的关系 |
|---|---|---|
| `E:\ZZZ\local-only\RemielleAssetVault`（19G） | 权威逆向库：`recovered/meshes/by-source`、`recovered/animations/highest-quality`（664 完整分层 ACL）、`standalone-native`（9 独立 ACL + 156 未压缩）、`scalar-recovered/full-source`、蒙皮数学推导 | 所有逆向资产的权威来源。动画/网格/材质的取证一律以此为准；新采集落 `20_ReverseEngineering/Raw` |
