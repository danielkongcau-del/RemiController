...
{
 public static class SourceSpecialSmokeBuild
 {
  ...
  public static void Run()
  {
   ...
   try
   {
    ...
    var prefabs=JObject.Parse(pack)["effects"].Select(e=>AssetDatabase.LoadAssetAtPath<GameObject>("Assets/ControllerIntegration/Effects/"+((string)e["kind"]=="smoke"?"SpecialSmoke/":(string)e["kind"]=="burst"?"SpecialBurst/":"SpecialFlash/")+(string)e["prefabName"]+".prefab")).ToArray();
    foreach(int hz in new[]{30,60,120})
    {
     ...
     if(fx.BurstActivations!=2||fx.Prewarmed!=30)throw new Exception("Paired bursts failed");
     ...
     if(fx.ActiveCount!=4||fx.BurstCancelled!=0)throw new Exception("First-frame-follow burst was cancelled with actor action");
   ...
   if(!(bool)report["pass"])throw new Exception("Smoke lifecycle failed");
