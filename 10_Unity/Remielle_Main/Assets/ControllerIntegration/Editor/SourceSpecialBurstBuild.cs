 ...
 public static class SourceSpecialBurstBuild
 {
  ...
  public static void Run()
  {
   ...
   try
   {
    ...
    string pack=File.ReadAllText("Assets/ControllerIntegration/Data/source-special-particles-runtime.json");
    var prefabs=JObject.Parse(pack)["effects"].Select(e=>AssetDatabase.LoadAssetAtPath<GameObject>("Assets/ControllerIntegration/Effects/"+((string)e["kind"]=="burst"?"SpecialBurst/":"SpecialFlash/")+(string)e["prefabName"]+".prefab")).ToArray();
    foreach(int hz in new[]{30,60,120})
    {
     ...
     Notice(SourceActionPhase.Enter,1,0);Notice(SourceActionPhase.Sample,1,17);fx.Advance(1f/hz);
     if(fx.BurstActivations!=0)throw new Exception("Burst before frame 18");
     Notice(SourceActionPhase.Sample,1,18);fx.Advance(1f/hz);
     if(fx.BurstActivations!=2||fx.Prewarmed!=16)throw new Exception("Paired bursts failed");
     ...
     if(fx.ActiveCount!=2||fx.BurstCancelled!=0)throw new Exception("First-frame-follow burst was cancelled with actor action");
     ...
     {
      long id=cast+2;Notice(SourceActionPhase.Enter,id,0);Notice(SourceActionPhase.Sample,id,18);fx.Advance(0);
      Notice(SourceActionPhase.Leave,id,18);Notice(SourceActionPhase.Exit,id,18);
      for(int i=0;i<(int)Math.Round(hz*.6);i++)fx.Advance(1f/hz);
     }
     if(fx.Created!=fx.Prewarmed||fx.BurstActivations!=14)throw new Exception("Held special exhausted prewarm or lost a generation");
   ...
   if(!(bool)report["pass"])throw new Exception("Burst lifecycle failed");
