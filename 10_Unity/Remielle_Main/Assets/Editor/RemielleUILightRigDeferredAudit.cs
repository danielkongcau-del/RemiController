...
public static class RemielleUILightRigDeferredAudit
{
    ...
    const string Out = "E:/ZZZ/ZCode/90_Builds/RenderingReview/20260905/ui-live-binding/light-rig/deferred/";
    ...
    public static void Run()
    {
        ...
        try
        {
            foreach(string profileName in new[]{"display","store"})
            {
                ...
                using var geometry=new RemielleNativeUIRenderer(profile,driver,camera);var rig=new RemielleNativeUILightRig(profile);
                ...
                string[] samples={"__rest","Idle_Loop","Idle_Loop","Idle_Loop","Idle_Loop"};
                for(int sample=0;sample<5;sample++)
                {
                    RemielleUILightRigAudit.MoveModel(driver,sample);if(sample!=0){var state=driver.nativeAnimation[samples[sample]];if(state==null)throw new Exception("Required native action unavailable: "+samples[sample]);driver.Sample(state.name,state.length*.37f);}
                    int width=960,height=540;float yaw=0;
                    camera.aspect=(float)width/height;camera.fieldOfView=38;camera.nearClipPlane=.1f;camera.farClipPlane=50;
                    RemielleUILiveRasterAudit.FrameModel(geometry,camera,yaw);geometry.Prepare(width,height);var rigState=RemielleUILightRigAudit.Apply(rig,geometry,null,driver,sample);geometry.Render();lighting.Render(geometry,camera,rig);
                    ...
                    cases.Add(new JObject{["id"]=id,["width"]=width,["height"]=height,["sample"]=samples[sample],["yaw"]=yaw,["lightRig"]=rigState,["inputs"]=inputs,["mask"]=Ref(stem+"-mask.r8"),["constants"]=Ref(stem+"-cb0.buf"),["hdr"]=Ref(hdr),["auxiliary"]=Ref(aux)});
                }
            }
            File.WriteAllText(Out+"inputs.txt",cases.Count+"\n"+lines,new UTF8Encoding(false));
            var files=new JArray();foreach(string path in new[]{"Assets/Editor/RemielleUILightRigAudit.cs","Assets/RenderingReview/Runtime/RemielleNativeUILightRig.cs","Assets/RenderingReview/Runtime/RemielleNativeUIEntityBuffer.cs"})files.Add(Ref(path));foreach(string path in new[]{"Assets/Editor/RemielleUILightRigDeferredAudit.cs","Assets/RenderingReview/Runtime/RemielleNativeUILighting.cs","Assets/RenderingReview/Runtime/RemielleNativeUIRenderer.cs","Assets/Shaders/CapturedDeferredCharacter523.shader",vs,ps,Root+"captured-deferred-character-ui.json"})files.Add(Ref(path));
