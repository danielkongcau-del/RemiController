        ...
        try
        {
            foreach(string profileName in new[]{"display","store"})
            {
                int before=PrivateTargets();
                {
                    ...
                    using var geometry=new RemielleNativeUIRenderer(profile,driver,camera);var rig=new RemielleNativeUILightRig(profile);using var lighting=CreateLighting(profileName);using var late=new RemielleNativeUILateBody(lateProfile);
                    ...
                    try
                    {
                        for(int i=0;i<5;i++)
                        {
                            RemielleUILightRigAudit.MoveModel(driver,i);var clip=driver.nativeAnimation["Idle_Loop"];float time=clip.length*.37f,yaw=0;if(i==0){driver.ResetSourcePose();driver.ApplyPose();}else driver.Sample(clip.name,time);
                            ...
                            var jitter=Vector2.zero;geometry.Prepare(w,h,jitter);late.Prepare(geometry,driver);var rigState=RemielleUILightRigAudit.Apply(rig,geometry,late,driver,i);
                            geometry.Render();lighting.Render(geometry,camera,rig);
                            ...
                            lines.AppendLine(source["resources"].Count().ToString());var entityRefs=new JArray();foreach(var r0 in source["resources"]){var r=(JObject)r0.DeepClone();if((int)r["kind"]==0){var entity=late.Entities.Single(e=>e.Source.binding=="_Late"+((string)r["stage"]).ToUpperInvariant()+((string)r["slot"]).ToUpperInvariant());string path=stem+"-"+(string)r["stage"]+(string)r["slot"]+".buf";File.WriteAllBytes(path,entity.Bytes);r["path"]=path;entityRefs.Add(Ref(path));}lines.AppendLine(((string)r["stage"]=="vs"?0:1)+" "+((string)r["slot"]).Substring(1)+" "+r["kind"]+" "+r["width"]+" "+r["height"]+" "+r["mips"]+" "+r["layers"]+" "+r["format"]+" "+Q((string)r["path"]));}
                            ...
                            cases.Add(new JObject{["id"]=id,["profile"]=profileName,["sample"]=clip.name,["time"]=time,["yaw"]=yaw,["jitterPixels"]=new JArray(jitter.x,jitter.y),["vertex"]=Ref(stem+"-vb.buf"),["index"]=Ref(stem+"-ib.buf"),["constants"]=constants,["entities"]=entityRefs,["lightRig"]=rigState,["initialHdr"]=Ref(hdr),["depth"]=Ref(depth),["output"]=Ref(output),["depthStencilPreserved"]=true,["temporalAuxiliaryPreserved"]=true});
            ...
            var implementation=new JArray();foreach(string p in new[]{"Assets/Editor/RemielleUILightRigAudit.cs","Assets/RenderingReview/Runtime/RemielleNativeUILightRig.cs","Assets/RenderingReview/Runtime/RemielleNativeUIEntityBuffer.cs"})implementation.Add(Ref(p));foreach(string p in new[]{"Assets/Editor/RemielleUILightRigLateAudit.cs","Assets/RenderingReview/Runtime/RemielleNativeUILateBody.cs","Assets/RenderingReview/Runtime/RemielleNativeUIRenderer.cs","Assets/RenderingReview/Runtime/RemielleNativeUIConstants.cs","Assets/RenderingReview/Runtime/RemielleNativeUIMeshBinding.cs","Assets/RenderingReview/Runtime/RemielleNativeUILighting.cs","Assets/V3/Remielle_V3_Animated.prefab"})implementation.Add(Ref(p));
            File.WriteAllText(Out+"unity.json",new JObject{["schema"]="remielle-ui-live-late-body-readback-v1",["width"]=w,["height"]=h,["cases"]=cases,["lifecycle"]=lifecycle,["inputs"]=Ref(Out+"inputs.txt"),["initial"]=Ref(Out+"initial.txt"),["source"]=Ref(RemielleUILateBodyBuild.Root+"isolated-inputs.json"),["implementation"]=implementation,["boundary"]="Five movable native presentation light states per profile; original CPU light producer and continuous Player acceptance remain separate."}.ToString());
        }
