    ...
    public static void Run()
    {
        ...
        try
        {
            foreach(string name in new[]{"display","store"})
            {
                ...
                {
                    RemielleUILightRigAudit.MoveModel(driver,view);string sample=view==0?"__rest":"Idle_Loop";if(view==0){driver.ResetSourcePose();driver.ApplyPose();}else{var state=driver.nativeAnimation[sample];driver.Sample(sample,state.length*.37f);}
                    float yaw=0;JObject rigState=null;RemielleUILiveRasterAudit.FrameModel(geometry,camera,yaw);
                    for(int frame=0;frame<16;frame++){var jitter=temporal.BeginFrame(1920,1080,frame==0);geometry.Prepare(1920,1080,jitter);late.Prepare(geometry,driver);if(frame==0)rigState=RemielleUILightRigAudit.Apply(rig,geometry,late,driver,view);else rig.Apply(geometry,late,rig.Anchor,rig.Rotation,rig.MainMultiplier,rig.AmbientMultiplier);geometry.Render();lighting.Render(geometry,camera,rig);late.Draw(lighting.Hdr,geometry.Depth);temporal.Render(lighting.SampledDepth,lighting.Auxiliary,lighting.Hdr);}
                    ...
                    geometry.Prepare(1920,1080);late.Prepare(geometry,driver);rig.Apply(geometry,late,rig.Anchor,rig.Rotation,rig.MainMultiplier,rig.AmbientMultiplier);geometry.Render();lighting.Render(geometry,camera,rig);late.Draw(lighting.Hdr,geometry.Depth);post.Render(lighting.Hdr);
                    ...
                    cases.Add(new JObject{["id"]=id,["profile"]=name,["sample"]=sample,["yaw"]=yaw,["width"]=1920,["height"]=1080,["accumulatedFrames"]=16,["preview"]=Ref(png),["raw"]=Ref(raw),["temporalHdr"]=Ref(hdr),["noTaaPreview"]=Ref(noTaa),["lateBodyIntegrated"]=true,["lightRig"]=rigState});
                }
            }
            var files=new JArray();foreach(string p in new[]{"Assets/RenderingReview/Runtime/RemielleNativeUILightRig.cs","Assets/RenderingReview/Runtime/RemielleNativeUIEntityBuffer.cs","Assets/Editor/RemielleUILightRigAudit.cs"})files.Add(Ref(p));foreach(string p in new[]{"Assets/Editor/RemielleUILightRigPreviewBuild.cs","Assets/RenderingReview/Runtime/RemielleNativeUITemporal.cs","Assets/RenderingReview/Runtime/RemielleNativeUIPost.cs","Assets/RenderingReview/Runtime/RemielleNativeUIConstants.cs","Assets/RenderingReview/Runtime/RemielleNativeUIRenderer.cs","Assets/RenderingReview/Runtime/RemielleNativeUILighting.cs"})files.Add(Ref(p));
