...
public static class RemielleUILightRigRasterAudit
{
    const string Out="E:/ZZZ/ZCode/90_Builds/RenderingReview/20260905/ui-live-binding/light-rig/raster/";
    ...
    public static void Run()
    {
        ...
        try
        {
            foreach(string name in new[]{"display","store"})
            {
                ...
                using var renderer=new RemielleNativeUIRenderer(profile,driver,camera);var rig=new RemielleNativeUILightRig(profile);
                driver.ResetSourcePose();driver.ApplyPose();
                var clips=driver.nativeAnimation.Cast<AnimationState>().ToArray();
                var samples=Enumerable.Range(0,5).Select(i=>(name:i==0?"__rest":"Idle_Loop",time:i==0?0:driver.nativeAnimation["Idle_Loop"].length*.37f)).ToArray();
                for(int sampleIndex=0;sampleIndex<samples.Length;sampleIndex++)
                {
                    RemielleUILightRigAudit.MoveModel(driver,sampleIndex);var sample=samples[sampleIndex];if(sample.name!="__rest")driver.Sample(sample.name,sample.time);
                    float yaw=0;
                    ...
                    renderer.Prepare(width,height);var rigState=RemielleUILightRigAudit.Apply(rig,renderer,null,driver,sampleIndex);
                    ...
                    foreach(var draw in renderer.Draws)
                    {
                        ...
                        var entityRefs=new JArray();foreach(var r0 in source["resources"])
                        {
                            var r=(JObject)r0.DeepClone();if((int)r["kind"]==0){var entity=draw.entities.Single(e=>e.Source.binding=="_Seq"+((string)r["stage"]).ToUpperInvariant()+((string)r["slot"]).ToUpperInvariant());string path=stem+"-"+(string)r["stage"]+(string)r["slot"]+".buf";File.WriteAllBytes(path,entity.Bytes);r["path"]=path;entityRefs.Add(Ref(path));}
                            inputs.AppendLine(((string)r["stage"]=="vs"?0:1)+" "+((string)r["slot"]).Substring(1)+" "+r["kind"]+" "+r["width"]+" "+r["height"]+" "+r["mips"]+" "+r["layers"]+" "+r["format"]+" "+Quote((string)r["path"]));
                        }
                        ...
                        sampleRows.Add(new JObject{["id"]=id,["profile"]=name,["sample"]=sample.name,["time"]=sample.time,["yaw"]=yaw,["relative"]=draw.source.relative,["sourceID"]=draw.source.captureID,["vs"]=vs,["ps"]=ps,["vertices"]=draw.mesh.vertexCount,["vertex"]=Ref(stem+"-vb.buf"),["index"]=Ref(stem+"-ib.buf"),["constants"]=constants,["entities"]=entityRefs,["lightRig"]=rigState.DeepClone()});
                    }
            ...
            var files=new JArray();foreach(string p in new[]{"Assets/Editor/RemielleUILightRigAudit.cs","Assets/RenderingReview/Runtime/RemielleNativeUILightRig.cs","Assets/RenderingReview/Runtime/RemielleNativeUIEntityBuffer.cs"})files.Add(Ref(p));foreach(var p in new[]{"Assets/Editor/RemielleUILightRigRasterAudit.cs","Assets/Editor/RemielleUILiveAssetFactory.cs","Assets/Editor/RemielleUILiveProfileBuild.cs","Assets/RenderingReview/Runtime/RemielleNativeUIRenderer.cs","Assets/RenderingReview/Runtime/RemielleNativeUIProfile.cs","Assets/RenderingReview/Runtime/RemielleNativeUIConstants.cs","Assets/RenderingReview/Runtime/RemielleNativeUIMeshBinding.cs","Assets/V3/Remielle_V3_Animated.prefab",RemielleUILiveProfileBuild.Live+"runtime-profiles.json"})files.Add(Ref(p));
            File.WriteAllText(Out+"unity.json",new JObject{["schema"]="remielle-ui-live-raster-v1",["width"]=width,["height"]=height,["cases"]=cases,["implementationFiles"]=files,["inputs"]=Ref(Out+"inputs.txt"),["states"]=Ref(Out+"states.txt"),["boundary"]="Current skin and camera, 24 ordered native draws. Five movable native presentation light states; original CPU weather/light producer remains unverified."}.ToString());
